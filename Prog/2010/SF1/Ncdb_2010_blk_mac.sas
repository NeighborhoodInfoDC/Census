/**************************************************************************
 ...
 Modifications:
  ...
  10/10/11 added sf1 housing variables for dc	
**************************************************************************/
...
%macro Ncdb_2010_blk_mac( state );
  ...
  ** Start submitting commands to remote server **;
  ...
  %let freqvars = ;
  /*merge in relevant variables from H table from SF1 data for dc*/
	%if &state = dc %then %do;
		proc sql;
	 	 create table sf1 as
	 		 select 
	  			pl.* , sf.H5i2, sf.H5i3, sf.H5i4,
				sf.H5i5, sf.H5i6, sf.H5i7, sf.H5i8,
				sf.H4i4, sf.H4i2, sf.H4i3

			  from 
				census.Census_pl_2010_&state. as pl 
	  		full join 
				census.Census_sf1_2010_&state._blks as sf
			on
				sf.GeoBlk2010=pl.GeoBlk2010;
		quit;
	%end;
    ...
    ** Standard geography vars **;
    ...
    %if &state = dc %then %do;
      ...
      ** Add standard DC geographies for obs. w/valid, nonmissing blocks **;
      ...
      %let freqvars = anc2002 city cluster2000 cluster_tr2000 psa2004 geo2000 geo2010 ward2002 eor zip;
  ...
  %File_info( data=Ncdb.NCDB_2010_&state._blk, freqvars= )
