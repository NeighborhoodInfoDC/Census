...
x cd /pub/data/sf12010/Tools;  %let pgm=cnvtsf1;  filename pgm "&pgm..sas";
 ...
 %let path=%str(/pub/data/sf12010);
 *** libname sf12010 "/pub/data2/temp";      *<====Change this to same as path if data are NOT embargoed====;
 libname sf12010 "&path";      *<====Change this to same as path if data are NOT embargoed====;
 ...
 %include 'tablevars.sas';  *---this code defines the macro variables PHvars PHCTvars PHCTRvars and PCOvars  as generated by
 ...
 %macro doit(state,stab);
  ...
  filename geos pipe "unzip -a -p &path/zipfiles/&stab.2010.sf1.zip &stab.geo2010.sf1";   %*<======NOTE=============;
...
data
  ...
  retain State "&state" Stab "&stab";  length County $5 Tract $7;    *--and then these... -;
...
%do ifile=1 %to 47;
  ...
  filename in&ifile pipe "unzip -a -p &path/zipfiles/&stab.2010.sf1.zip &stab.000&i2.2010.sf1";   %*<======NOTE=============;
  _iseg="&i2";
...
cklogrecno:
  ...
  *---Stricly a Missouri thing to view a contents-directory report---;
  ...
  %contdir(sf12010);
   run;
  ods html close;
   %include sascode(notify);   *<---local utility for OSEDA use only.  Others can delete this line---;
