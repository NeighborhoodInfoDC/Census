/**************************************************************************
 ...
 files. Adapted from the previous Census Bureau program pl_all_3_2010.sas
 program. Census did not release a 2020 version of this program along with
 the data on 8/12/21. 
...
%macro pl_all_3_2020_mac( state );
  ...
  ***PT***infile 'the location of the files on your computer\msgeo2010.pl' lrecl = 400 missover pad;
  infile "&in_path\&state.geo&dec_yr..pl" lrecl = 400 missover pad;
  INPUT
  ...
  ***PT***data xxx.combine;
  data Census_pl_&dec_yr._&state ;
    ...
    ******/
      if not in1 then do; %err_put( macro=pl_all_3_2020_mac, msg='Logrecno is not in HEADER at line ' _n_ ) end;
      if not in2 then do; %err_put( macro=pl_all_3_2020_mac, msg='Logrecno is not in PART1 at line ' _n_ ) end;
      if not in3 then do; %err_put( macro=pl_all_3_2020_mac, msg='Logrecno is not in PART2 at line ' _n_ ) end;
    end;
